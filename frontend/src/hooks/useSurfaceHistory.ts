...
import { HistoricalSurfaceData, SurfaceHistoryState, SurfaceHistoryHookParams } from '../types/surface';
...
export function useSurfaceHistory({
  ...
  minDTE = 15, // Add minimum DTE filter
  maxDTE = 120, // Add maximum DTE filter
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      // Add DTE filtering to the URL
      const url = `${API_CONFIG.baseUrl}${API_CONFIG.endpoints.volSurfaceHistory}?limit=${limit}&min_dte=${minDTE}&max_dte=${maxDTE}`;
